...
modulepath = os.path.split(os.path.realpath(__file__))[0]
rootDir = '/tmp/epub-builder'
...
class EpubBook:
    ...
    def addHtmlForImage(self, imageItem):
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/image.html'))
    ...
    def remoteToLocalImageUrls(self, urls, html):
        for url in urls:
            html = html.replace(url, os.path.join('images', os.path.split(url)[1]))
        return html
    ...
    def __makeTitlePage(self):
        ...
        if self.titlePage.html:
            return
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/title-page.html'))
    ...
    def addTitlePage(self, html = ''):
        ...
        self.addGuideItem(os.path.join('/tmp' , 'epub-builder' , 'title-page.html'), 'Title Page', 'title-page')

    def __makeTocPage(self):
        assert self.tocPage
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/toc.html'))
        stream = tmpl.generate(book = self)
    ...
    def addTocPage(self):
        ...
        self.addGuideItem(os.path.join('/tmp' , 'epub-builder' , 'toc.html'), 'Table of Contents', 'toc')
    ...
    def __writeContainerXML(self):
        ...
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/container.xml'))
    ...
    def __writeTocNCX(self):
        ...
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/toc.ncx'))
        stream = tmpl.generate(book = self)
        fout.write(stream.render('xml'))
        fout.close()
    ...
    def __writeContentOPF(self):
        ...
        tmpl = self.loader.load(os.path.join(modulepath, 'templates/content.opf'))
        stream = tmpl.generate(book = self)
        fout.write(stream.render('xml'))
        fout.close()
